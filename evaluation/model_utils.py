...
def generate_completions(
    ...
    **generation_kwargs
    ...
    返回：
        generations: 生成的文本
        output_token_ids: 仅生成部分的token id（不含prompt）
        prompt_token_ids: prompt部分的token id
    """
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        # batch_outputs: [batch, seq_len]

        # 处理每个样本
        for j in range(batch_outputs.shape[0]):
            output_ids = batch_outputs[j].tolist()
            prompt_len = prompt_lens[j]
            prompt_ids = batch_input_ids[j].tolist()
            # 生成部分的token id（去除prompt部分）
            gen_ids = output_ids[prompt_len:]
            all_output_token_ids.append(gen_ids)
            all_prompt_token_ids.append(prompt_ids)
            # 解码文本
            gen_text = tokenizer.decode(output_ids, skip_special_tokens=True)
            prompt_text = tokenizer.decode(prompt_ids, skip_special_tokens=True)
            # 去除prompt部分
            if gen_text.startswith(prompt_text):
                gen_text_only = gen_text[len(prompt_text):]
            else:
                gen_text_only = gen_text
            # 去除stop sequence
            if stop_id_sequences is not None:
                for stop_seq in stop_id_sequences:
                    # stop_seq为id序列，需转为文本
                    stop_str = tokenizer.decode(stop_seq, skip_special_tokens=True)
                    if stop_str and stop_str in gen_text_only:
                        gen_text_only = gen_text_only.split(stop_str)[0]
            generations.append(gen_text_only)

        if not disable_tqdm:
            progress.update(batch_outputs.shape[0])

    assert len(generations) == len(prompts) * num_return_sequences, "number of generations should be equal to number of prompts * num_return_sequences"
    if return_token_ids or return_prompt_token_ids:
        # output_token_ids为生成部分，prompt_token_ids为prompt部分
        return generations, all_output_token_ids, all_prompt_token_ids
    else:
        return generations
