...
def construct_prompt(example, data_name, args):
    # Determine whether to use adaptive few-shot (few-shot for multiple-choice, zero-shot for others), and specify the number of demonstration samples
    if args.adapt_few_shot and data_name in [
        ...
        # For these datasets, always use 5 demonstration samples
        demos = load_prompt(data_name, args.prompt_type, 5)
    else:
        # Otherwise, use the number of shots specified by the parameter
        demos = load_prompt(data_name, args.prompt_type, args.num_shots)
    prompt_type = args.prompt_type
    # Special case: if prompt_type is platypus_fs, actually use cot template
    if prompt_type == "platypus_fs":
        prompt_type = "cot"
    # Special case: if prompt_type is tool-integrated, actually use tora template
    if prompt_type == "tool-integrated":
        prompt_type = "tora"

    # Get the template for the current prompt_type
    prompt_temp = PROMPT_TEMPLATES[args.prompt_type]

    # Get the splitter, input template, and output template from the template
    ...
    # Special handling for qwen25-math-cot: merge all demonstration samples into one turn
    if args.prompt_type == "qwen25-math-cot":
        # Hotfix: merge all demonstration samples into one turn
        demo_prompt = splitter.join([q + "\n" + a for q, a in demos])
    else:
        # Standard concatenation: format each demonstration sample with input and output templates and join
    ...
    # Construct the input content for the current sample
    context = input_template.format(input=example["question"])
    # If there are no demonstration samples, or adaptive few-shot but the current sample is not multiple-choice, use only context
    if len(demo_prompt) == 0 or (
        args.adapt_few_shot and example.get("gt_ans", None) not in ["A", "B", "C", "D", "E"]
    ):
        full_prompt = context
    else:
        if args.prompt_type == "qwen25-math-cot":
            # Hotfix: merge all demonstration samples into one turn, then append the current question
            full_prompt = demo_prompt + splitter + example["question"]
            full_prompt = input_template.format(input=full_prompt)
        else:
            # Standard concatenation: demonstration samples + splitter + current question
            full_prompt = demo_prompt + splitter + context

    # For platypus_fs type, wrap with an instruction template
    ...
    # For tool-integrated (tora) type, add an English instruction header
    if prompt_type == "tora":
        full_prompt = (
...
"""
    ...
    # Remove leading and trailing spaces to avoid formatting issues
