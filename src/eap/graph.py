...
class Node:
    """
    Node class in the computation graph. in_hook is the input hook in TL (TransformerLens), out_hook is the output hook.
    """
    name: str
    layer: int
    in_hook: str
    ...
    # The in_graph/score/neurons_in_graph/neurons_scores state of the node is directly obtained from the graph
    @property
    def in_graph(self):
...
class Edge:
    ...
    Attributes:
        ...
        score: (Optional[float]) Score of the edge (given by attribution method)
        in_graph: (bool) Whether the edge is in the graph
    """
    ...
    def __init__(self, graph: 'Graph', parent: Node, child: Node, qkv: Optional[Literal["q", "k", "v"]] = None):
        ...
        self.child = child
        self.qkv = qkv
        # matrix_index is used to locate the edge in the tensor
    ...
    @in_graph.setter
    def in_graph(self, value):
        # Set whether the edge is in the graph
...
class Graph:
    """
    Computation graph class, contains nodes and edges.
    ...
    in_graph: torch.Tensor  # (n_forward, n_backward) Boolean tensor indicating if edge is in the graph
    neurons_scores: Optional[torch.Tensor]  # (n_forward, d_model) Neuron scores
    neurons_in_graph: Optional[torch.Tensor]# (n_forward, d_model) Boolean tensor for neurons in the graph
    nodes_scores: Optional[torch.Tensor]    # (n_forward) Node scores
    nodes_in_graph: torch.Tensor            # (n_forward) Boolean tensor for nodes in the graph
    forward_to_backward: torch.Tensor       # (n_forward, n_backward) Mapping from forward to backward
    real_edge_mask: torch.Tensor            # (n_forward, n_backward) Whether it is a real edge
    cfg: GraphConfig
    ...
    def forward_index(self, node: Node, attn_slice=True) -> int:
        # Get the forward index of a node
    ...
    def backward_index(self, node: Node, qkv=None, attn_slice=True) -> int:
        # Get the backward index of a node
        return Graph._backward_index(self.cfg, node.name, qkv, attn_slice)

    def get_dst_nodes(self) -> List[str]:
        # Get all destination node names (including qkv of attention heads, mlp, and logits)
        heads = []
    ...
    def weighted_edge_count(self) -> float:
        """
        Compute weighted edge count (if neuron mask exists, weight by number of neurons, otherwise count edges)
        Returns:
    ...
    def apply_threshold(self, threshold: float, absolute: bool = True, reset: bool = True, level: Literal['edge', 'node', 'neuron'] = 'edge', prune=True):
        """
        Select by threshold, add edges/nodes/neurons with score greater than threshold to the graph. Unscored nodes/neurons are kept by default.
        Args:
    ...
    def apply_topn(self, n: int, absolute: bool = True, level: Literal['edge', 'node', 'neuron'] = 'edge', reset: bool = True, prune: bool = True):
        """
        Keep only the top-n edges/nodes/neurons. The level determines the selection granularity.
        Args:
    ...
    def apply_greedy(self, n_edges: int, absolute: bool = True, reset: bool = True, prune: bool = True):
        """
        Use a greedy algorithm to select top-n edges from logits upwards. Only applicable at edge level.
        Args:
    ...
    def prune(self):
        """
        Prune the graph to ensure it is connected. Remove useless nodes/edges/neurons.
        Steps:
        ...
        while not old_new_same:
            ...
            nodes_with_outgoing = self.in_graph.any(dim=1)
            nodes_with_ingoing = einsum(self.in_graph.any(dim=0).float(), self.forward_to_backward.float(), 'backward, forward backward -> forward') > 0
            nodes_with_ingoing[0] = True  # Input node is always considered to have incoming edges
    ...
    def from_model(cls, model_or_config: Union[HookedTransformer, HookedTransformerConfig, Dict], neuron_level: bool = False, node_scores: bool = False) -> 'Graph':
        """
        Instantiate Graph from a model or config object.
        Args:
    ...
    def from_json(cls, json_path: str) -> 'Graph':
        ...
        Note: Not suitable for huge graphs at neuron resolution.
        """
    ...
    def from_pt(cls, pt_path: str) -> 'Graph':
        ...
        The file must contain:
            ...
            5. 'edges_in_graph': whether edge is in the graph
