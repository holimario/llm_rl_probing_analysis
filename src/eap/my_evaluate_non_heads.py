...
from .my_graph_non_heads import Graph, AttentionNode
...
def evaluate_graph(
    ...
    """
    Evaluate a circuit (i.e., a graph with only some nodes set to False, usually created by calling graph.apply_threshold).
    Pruning is usually required first to ensure the circuit is valid.
    ...
    Args:
        ...
        intervention (Literal['patching', 'zero', 'mean','mean-positional'], optional): Type of intervention.
            'patching' is swap intervention; 'mean-positional' takes mean by position over the given dataset. Default is 'patching'.
        intervention_dataloader (Optional[DataLoader], optional): Dataset for computing means, only needed for mean interventions.
    ...
    if graph.neurons_in_graph is not None:
        ...
        # If an edge is in the graph but not all its neurons are, still need to update that edge
    ...
    # # Invert the matrix as a mask to specify which edges should be "destroyed"
    ...
    # For each node, construct an input hook. If the corresponding edge is not in the graph, "destroy" its input
    ...
    def make_input_construction_hook(activation_matrix, in_graph_vector, neuron_matrix):
        def input_construction_hook(activations, hook):
            ...
            if model.cfg.use_normalization_before_and_after:
                ...
                # Get clean outputs of all previous attention heads
                ...
                # Compute update for non-attention heads and the difference for clean and corrupted attention heads
                if neuron_matrix is not None:
                ...
                # Both clean and corrupted attention results go through layernorm, and the difference is added to update
    ...
    def make_input_construction_hooks(activation_differences, in_graph_matrix, neuron_matrix):
        ...
        for layer in range(model.cfg.n_layers):
            ...
            if graph.nodes[f'a{layer}'].in_graph and \
                not (neuron_matrix is None and all(parent_edge.in_graph for parent_edge in graph.nodes[f'a{layer}'].parent_edges)):
            ...
            # If the MLP node in this layer is in the graph, also add the MLP hook
    ...
    # If metrics is not a list, convert it to a list
    ...
    for all_tokens, answer_range, origin_answer_token_ids in dataloader:
        # Tokenize clean and corrupted samples separately
    ...
    # If only one metric, return the tensor directly
    if len(results) == 1:
        results = results[0]
    return results
