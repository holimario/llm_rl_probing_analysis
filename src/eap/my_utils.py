...
from .my_graph import Graph, AttentionNode, LogitNode


def token_padding(model: HookedTransformer, inputs: List[str], padding_token_id: int):
    """
    Tokenize a list of input strings using the given model, and return the tokenized results, attention mask, input lengths, and maximum sequence length.

    Args:
        model (HookedTransformer): The model instance used for tokenization.
        inputs (List[str]): List of string id sequences of varying lengths.
        padding_token_id (int): The token id used for padding.

    Returns:
        tuple: A tuple containing:
            - tokens (torch.Tensor): The tokenized tensor, shape [batch, seq_len].
            - attention_mask (torch.Tensor): The attention mask, 1 for valid tokens, 0 for padding, shape [batch, seq_len].
            - input_lengths (torch.Tensor): The number of valid tokens for each input, shape [batch].
            - n_pos (int): The maximum sequence length in the current batch (i.e., seq_len).
    """
...
def tokenize_plus(model: HookedTransformer, inputs: List[str], max_length: Optional[int] = None):
    ...
    Args:
        model (HookedTransformer): The model instance used for tokenization.
        inputs (List[str]): List of strings to be tokenized.
    ...
    if max_length is not None:
        # If max_length is specified, temporarily modify the model's n_ctx parameter to fit the tokenization length
    ...
    if max_length is not None:
        # Restore the model's original n_ctx parameter
...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int, scores: Optional[Tensor]):
    ...
    Args:
        ...
        batch_size (int): The size of the batch being attributed.
        n_pos (int): The size of the position dimension.
        scores (Tensor): The scores tensor to fill. If None, hooks/matrices are for evaluation only (do not use backward hooks).

    Returns:
        Tuple[Tuple[List, List, List], Tensor]: The final tensor ([batch, pos, n_src_nodes, d_model]) stores activation differences, i.e. corrupted - clean activations. The first set of hooks adds activations (run on corrupted input), the second subtracts activations (run on clean input), and the third computes gradients and updates the scores matrix.
    """
...
def compute_mean_activations(model: HookedTransformer, graph: Graph, dataloader: DataLoader, per_position=False):
    ...
    def activation_hook(index, activations, hook, means=None, input_lengths=None):
        # Defines a hook that will fill up our means tensor. Means is of shape
    ...
    # Get all the out hooks / indices needed for making hooks
    ...
    for batch in tqdm(dataloader, desc='Computing mean'):
        # The dataset may be given as a tuple, or just raw strings
...
def make_hooks_and_matrices_test1(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int, scores: Optional[Tensor], output_activation: bool=False):
    """
    Construct the activation difference matrix and forward/backward hook lists for attribution.

    Args:
        model (HookedTransformer): The model to attribute.
        graph (Graph): The graph structure for attribution.
        batch_size (int): The number of samples in the current batch.
        n_pos (int): The sequence length (number of positions).
        scores (Tensor): The score tensor to be filled. If None, only used for evaluation (no backward hooks).

    Returns:
        Tuple[Tuple[List, List, List], Tensor]: 
            - Two hook lists (forward hooks, backward hooks)
            - The activation difference tensor, shape [batch, pos, n_src_nodes, d_model], storing corrupted - clean activations
    """
    ...
    def gradient_hook(prev_index: int, bwd_index: Union[slice, int], gradients:torch.Tensor, hook):
        ...
        Args:
            prev_index (int): The forward index of the previous node.
            bwd_index (int or slice): The backward index of the current node.
            gradients (Tensor): The gradients from the current backward pass.
            hook: Unused.
        """
        grads = gradients.detach()
        try:
            # If the gradient is 3D, unsqueeze to match einsum requirements
            if grads.ndim == 3:
                grads = grads.unsqueeze(2)
            # Use einsum to compute: activation difference times gradient, accumulate to scores
    ...
    # Iterate over each layer, register attention and MLP node forward/backward hooks
    for layer in range(graph.cfg['n_layers']):
    ...
    # Register backward hook for logits node only
    node = graph.nodes['logits']
    prev_index = graph.prev_index(node)
    bwd_index = graph.backward_index(node)
    bwd_hooks.append((node.in_hook, partial(gradient_hook, prev_index, bwd_index)))
