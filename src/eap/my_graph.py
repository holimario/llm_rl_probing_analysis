...
class Edge:
    ...
    def __init__(self, graph: 'Graph', parent: Node, child: Node, qkv: Optional[Literal["q", "k", "v"]] = None):
        self.graph = graph
        # Name the edge, attention edges have <q>/<k>/<v>
...
class Graph:
    ...
    neurons_in_graph: Optional[torch.Tensor]# (n_forward, d_model) Whether neuron is in the graph
    nodes_scores: Optional[torch.Tensor]    # (n_forward) Node scores
    nodes_in_graph: torch.Tensor            # (n_forward) Whether node is in the graph
    ...
    def _n_forward(cls, cfg) -> int:
        # Calculate number of forward nodes
        return 1 + cfg.n_layers * (cfg.n_heads + 1)

    @classmethod
    def _n_backward(cls, cfg) -> int:
        # Calculate number of backward nodes
    ...
    def _forward_index(cls, cfg, node_name: str, attn_slice: bool = False) -> int:
        ...
        if node_name == 'input':
            ...
            return slice(i, i + cfg.n_heads) if attn_slice else i + head  # This is a slice for all heads, for convenience
        else:
    ...
    def get_dst_nodes(self) -> List[str]:
        # Get all destination node names (including qkv of attention heads, mlp, logits)
        heads = []
    ...
    def weighted_edge_count(self) -> float:
        """
        Calculate weighted edge count (if neuron mask exists, weight by neuron count, otherwise count edges)
        Returns:
    ...
    def reset(self, empty=True):
        ...
        Args:
            empty (bool): True means clear the graph, False means select all
        """
        if empty:
    ...
    def apply_topn(self, n: int, absolute: bool = True, level: Literal['edge', 'node', 'neuron'] = 'edge', reset: bool = True, prune: bool = True):
        """
        Keep only the top-n edges/nodes/neurons. Level determines the selection granularity.
        Args:
    ...
    def apply_greedy(self, n_edges: int, absolute: bool = True, reset: bool = True, prune: bool = True):
        """
        Use greedy algorithm to select top-n edges from logits upwards. Only applicable to edge level.
        Args:
    ...
    def prune(self):
        ...
        while not old_new_same:
            ...
            nodes_with_ingoing[0] = True  # Input node is always considered to have incoming edge
