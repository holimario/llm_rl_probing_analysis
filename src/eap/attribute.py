    ...
    for clean, corrupted, label in dataloader:
        ...
        with torch.inference_mode():
            if intervention == 'patching':
                ...
                # zero/mean ablation: do not add corrupted activations, but add mean if needed
...
def get_scores_ig_activations(model: HookedTransformer, graph: Graph, dataloader: DataLoader, 
    ...
    """
    Get scores using Integrated Gradients with activation interpolation.
    ...
    for clean, corrupted, label in dataloader:
        ...
        # Get hooks and activation differences
        ...
        # Build list of nodes to interpolate
...
def attribute(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
    ...
    """
    Main attribution function, selects the attribution implementation according to the method.
    ...
    if method == 'EAP':
        ...
        if intervention != 'patching':
            raise ValueError(f"EAP-IG-inputs method only supports 'patching' intervention, got {intervention}")
        scores = get_scores_eap_ig(
            model, graph, dataloader, metric, 
            steps=ig_steps, quiet=quiet, device=device
        )
    elif method == 'clean-corrupted':
        if intervention != 'patching':
            raise ValueError(f"clean-corrupted method only supports 'patching' intervention, got {intervention}")
        ...
        scores = get_scores_ig_activations(
            model, graph, dataloader, metric, 
            steps=ig_steps, intervention=intervention, 
            intervention_dataloader=intervention_dataloader, 
            quiet=quiet, device=device
        )
    elif method == 'information-flow-routes':
        scores = get_scores_information_flow_routes(
            model, graph, dataloader, quiet=quiet, device=device
        )
    elif method == 'exact':
        ...
        raise ValueError(
            f"method must be one of ['EAP', 'EAP-IG-inputs', 'clean-corrupted', 'EAP-IG-activations', 'information-flow-routes', 'exact'], got {method}"
        )
