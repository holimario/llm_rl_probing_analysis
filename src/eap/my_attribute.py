...
from .my_evaluate import evaluate_graph, evaluate_baseline
...
def get_scores_exact(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
    ...
    for edge in edges:
        edge.in_graph = False  # Remove the current edge
    ...
    for clean, corrupted, label in dataloader:
        ...
        with torch.inference_mode():
            if intervention == 'patching':
                ...
                # zero/mean ablation: do not add corrupted activations, but add means for mean
