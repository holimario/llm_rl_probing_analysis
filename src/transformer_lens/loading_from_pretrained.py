...
from src.transformer_lens.pretrained.weight_conversions import (
...
def get_pretrained_state_dict(
    ...
    **kwargs: Any,
    ...
    """
    Load the weights of a pretrained model and process them into the parameter names and shapes required by HookedTransformer.
    Supports checkpointed models (checkpoint information should be stored in the config object).

    Args:
        official_model_name: Official model name or local path
        cfg: HookedTransformerConfig configuration object
        hf_model: Optional, HuggingFace model object. If provided, it will be used directly without reloading
        dtype: torch dtype to load the model weights as
        kwargs: Other optional arguments passed to HuggingFace from_pretrained, also passed to other compatible HuggingFace functions
    """
    # If torch_dtype is specified in kwargs, override dtype and remove from kwargs
    if "torch_dtype" in kwargs:
        dtype = kwargs["torch_dtype"]
        del kwargs["torch_dtype"]
    # Check if official_model_name is a local path, if so, convert to absolute path
    if Path(official_model_name).exists():
        ...
        # Otherwise, get the official model name (may be an alias)
        official_model_name = get_official_model_name(official_model_name)
    # Some models require trust_remote_code=True, warn and set automatically if not set
    if official_model_name.startswith(NEED_REMOTE_CODE_MODELS) and not kwargs.get(
        "trust_remote_code", False
    ):
        logging.warning(
        ...
        kwargs["trust_remote_code"] = True
    # Handle models from NeelNanda, ArthurConmy, Baidicoot, etc. (weights stored as .pth files in HF repo)
    if (
        official_model_name.startswith("NeelNanda")
        or official_model_name.startswith("ArthurConmy")
        or official_model_name.startswith("Baidicoot")
        ...
        api = HfApi()
        # Get all file names in the repo
        repo_files = api.list_repo_files(
            official_model_name,
            **utils.select_compatible_kwargs(kwargs, api.list_repo_files),
        )
        # If checkpoint is specified, find the corresponding checkpoint file, otherwise find final.pth
        ...
        # Handle HuggingFace official models
        if cfg.from_checkpoint:
            # Get HuggingFace Token (if any)
            huggingface_token = os.environ.get("HF_TOKEN", "")
            # Stanford CRFM checkpoint naming
            if official_model_name.startswith("stanford-crfm"):
            ...
            # EleutherAI/pythia checkpoint naming
            elif official_model_name.startswith("EleutherAI/pythia"):
                hf_model = AutoModelForCausalLM.from_pretrained(
                    official_model_name,
                ...
                # Other models do not support checkpoint loading yet
                raise ValueError(f"Checkpoints for model {official_model_name} are not supported")
        elif hf_model is None:
            # If hf_model object is not provided, automatically load from HuggingFace
            huggingface_token = os.environ.get("HF_TOKEN", "")
            # If model is not hosted on HuggingFace, raise exception
            ...
            # For all other models, use AutoModelForCausalLM
            else:
                hf_model = AutoModelForCausalLM.from_pretrained(
                    official_model_name,
                    torch_dtype=dtype,
        ...
        # Freeze model parameters (do not require gradients)
        for param in hf_model.parameters():
            param.requires_grad = False

        # Call different weight conversion functions according to original architecture type
        if cfg.original_architecture == "GPT2LMHeadModel":
            ...
            # Unsupported architecture type, raise exception and prompt
            raise ValueError(
