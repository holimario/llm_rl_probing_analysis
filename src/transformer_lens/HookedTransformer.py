...
import src.transformer_lens.loading_from_pretrained as loading
import src.transformer_lens.utils as utils
from src.transformer_lens.ActivationCache import ActivationCache
from src.transformer_lens.components import (
    Embed,
    LayerNorm,
    LayerNormPre,
    PosEmbed,
...
from src.transformer_lens.FactoredMatrix import FactoredMatrix
from src.transformer_lens.hook_points import HookedRootModule, HookPoint
from src.transformer_lens.HookedTransformerConfig import HookedTransformerConfig
from src.transformer_lens.loading_from_pretrained import NON_HF_HOSTED_MODEL_NAMES

# Note - activation cache is used with run_with_cache, past_key_value_caching is used for
# generation.
from src.transformer_lens.past_key_value_caching import HookedTransformerKeyValueCache
from src.transformer_lens.utilities import devices
from src.transformer_lens.utils import (
    USE_DEFAULT_VALUE,
...
class HookedTransformer(HookedRootModule):
    ...
    def __init__(
        ...
        Note: If you want to load the model from pretrained weights, you should use :meth:`from_pretrained`.

        Args:
            cfg: The configuration for the model, can be a HookedTransformerConfig or a dict.
            tokenizer: The tokenizer for the model. If not provided, it will be inferred from cfg.tokenizer_name or set to None.
                       If None, the model cannot process strings directly, and d_vocab must be explicitly set.
            move_to_device: Whether to move the model to the device specified by cfg.device.
                            If n_devices > 1 in config, this must be True, as model layers will be distributed across devices.
            default_padding_side: Padding direction, either "left" or "right".
        """
        super().__init__()
        # Check cfg type, must not be a string, should be a config dict or HookedTransformerConfig object
        if isinstance(cfg, str):
            raise ValueError(
                "Please pass a config dict or HookedTransformerConfig object. If you want to load a pretrained model, use HookedTransformer.from_pretrained()."
            )
        ...
        if tokenizer is not None:
            # If user provides a tokenizer, set it directly
            self.set_tokenizer(tokenizer, default_padding_side=default_padding_side)
        elif self.cfg.tokenizer_name is not None:
            # If config has a tokenizer name, try to load from HuggingFace
            if self.cfg.tokenizer_name in NON_HF_HOSTED_MODEL_NAMES:
                # Some models are not hosted on HuggingFace and need to be loaded manually
                logging.warning(
                    "%s tokenizer not loaded, please load manually.",
                    self.cfg.tokenizer_name,
                )
            else:
                # HuggingFace uses use_fast=True by default
                use_fast = True
                # For phi models, fast tokenizer does not support adding BOS token, so set use_fast=False
            ...
            # If no tokenizer name, assume algorithmic task, pass in tokens directly, no tokenizer needed
            assert self.cfg.d_vocab != -1, "If no tokenizer is provided, d_vocab must be explicitly set"
            self.tokenizer = None
            if default_padding_side != "right":
                logging.warning(
                    "default_padding_side was explicitly set but ignored because no tokenizer is set."
                )

        # Embedding layer, for input token embeddings
        self.embed = Embed(self.cfg)
        self.hook_embed = HookPoint()  # [batch, pos, d_model], for embedding layer hook

        # Positional embedding layer (only created if not rotary type)
        if self.cfg.positional_embedding_type != "rotary":
            self.pos_embed = PosEmbed(self.cfg)
            self.hook_pos_embed = HookPoint()  # [batch, pos, d_model], for positional embedding hook

        # Whether to use hook tokens
        if self.cfg.use_hook_tokens:
            self.hook_tokens = HookPoint()  # [batch, pos], for hook tokens

        # List of Transformer blocks, each is a TransformerBlock
        self.blocks = nn.ModuleList(
            [TransformerBlock(self.cfg, block_index) for block_index in range(self.cfg.n_layers)]
        )

        # Final normalization layer, choose type based on config
        if self.cfg.normalization_type == "RMS":
            ...
            # If LNPre and final_rms is True, use RMSNormPre, else use LayerNormPre
            if self.cfg.final_rms:
                self.ln_final = RMSNormPre(self.cfg)
            else:
                self.ln_final = LayerNormPre(self.cfg)
        elif self.cfg.normalization_type is None:
            # If None, do not create normalization layer
            pass
        else:
            logging.warning("Invalid normalization_type passed: %s", self.cfg.normalization_type)
        ...
        # Whether to move model to specified device (supports multi-GPU pipeline parallelism)
        if move_to_device:
            # Assign modules to devices in pipeline fashion: first device gets embedding and positional embedding layers and first n_layers // n_devices blocks,
            # second device gets next n_layers // n_devices blocks, and so on.
            # Last device gets the last block, normalization layer, and output layer
            self.move_model_modules_to_device()

        # Helper variable for storing a small (10K-20K) training dataset, default is None, can be loaded via load_sample_training_dataset
        self.dataset = None

        # Assign parameter names to each module (relative to root module), required for HookPoints
        self.setup()
    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        """
        # Check if the model is a T5 model; if so, raise an exception and instruct the user to use HookedEncoderDecoder instead.
        ...
        # Check if 8bit or 4bit quantization is requested; currently not supported, so assert failure if so.
        assert not (
            from_pretrained_kwargs.get("load_in_8bit", False)
            or from_pretrained_kwargs.get("load_in_4bit", False)
        ), "Quantization not supported"

        # If the user provides a HuggingFace model object (hf_model), extract config information from it.
        if hf_model is not None:
            assert hf_model.config is not None  # Ensure config exists
            hf_cfg = hf_model.config.to_dict()  # Convert to dict
            qc = hf_cfg.get("quantization_config", {})  # Get quantization config
            load_in_4bit = qc.get("load_in_4bit", False)
            load_in_8bit = qc.get("load_in_8bit", False)
            quant_method = qc.get("quant_method", "")
            # Assert 8bit quantization is not supported
            assert not load_in_8bit, "8-bit quantization is not supported"
            # Assert 4bit quantization is only supported for torch>=2.1.1
            assert not (
            ...
            # Assert 4bit quantization is only supported for Llama models
            assert not (
                load_in_4bit and ("llama" not in model_name.lower())
            ), "Quantization is only supported for Llama models"
            # Assert 4bit quantization is only supported for bitsandbytes method
            if load_in_4bit:
                assert (
        ...
        # If dtype is a string, convert it to torch dtype
        if isinstance(dtype, str):
            dtype = DTYPE_FROM_STRING[dtype]
        # For backward compatibility: if torch_dtype is in from_pretrained_kwargs, override dtype
        if "torch_dtype" in from_pretrained_kwargs:
            dtype = from_pretrained_kwargs["torch_dtype"]

        # Warn if dtype is float16 and device is CPU
        if (
        ...
        # Get the official HuggingFace model name (remove aliases)
        official_model_name = loading.get_official_model_name(model_name)

        # Load model config, returns a HookedTransformerConfig object
        # If loading from checkpoint, cfg will contain checkpoint info
        ...
        # For models with shortformer positional embedding, fold_ln/center_unembed/center_writing_weights are not supported.
        # Issue warnings and force them to False.
        ...
        # If the model uses logit softcap and center_unembed is True, warn and force center_unembed to False
        ...
        # Load the model's state_dict (mapping from parameter names to tensors) and process it for HookedTransformer
        state_dict = loading.get_pretrained_state_dict(
            official_model_name, cfg, hf_model, dtype=dtype, **from_pretrained_kwargs
        )

        # Create the HookedTransformer object, but do not move to device yet
        model = cls(
            cfg,
            tokenizer,
        ...
        # Load and process the state_dict, including optional fold_ln, centering, etc.
        ...
        # If move_to_device is specified, move model modules to the target device
        if move_to_device:
